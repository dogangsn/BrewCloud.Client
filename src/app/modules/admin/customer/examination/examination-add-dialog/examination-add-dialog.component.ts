  ...
  getExamination(): Observable<any> {
    ...
    getPatients(): Observable<any> {
        ...
        setPatients(response: any): void {
            if (response.data) {
                debugger
                response.data = response.data.filter(x => x.id == this.examination.patientId);
...
fillFormData(selectedExaminationInf: ExaminationDto) {
    ...
    if (this.examination !== null) {
        ...
        this.symptoms = this.examination.symptoms.split(',').map(symptom => symptom.trim());
        this.examinationForm.setValue({
            ...
            selectedState: this.examination.status
          });
    }
}
